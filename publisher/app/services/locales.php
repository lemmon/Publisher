...
class Locales
{
    ...
    {
        $res = [];
        foreach (Pages::find(['site_id' => defined('SITE_ID') ? SITE_ID : null, '(parent_id IS NULL OR parent_id = -1)', 'top' => 1])->all() as $page) {
            $res[$page->locale_id] = [
    ...
    static function fetchInactiveWithPreferred()
    {
        if (!array_key_exists('preferred-inactive', self::$_cache))
        {
            $preferred = self::fetchAllWithPreferred();
            unset($preferred['active']);
            /* */
            return self::$_cache['preferred-inactive'] = $preferred;
        }
        else
        {
            return self::$_cache['preferred-inactive'];
        }
    }
}
